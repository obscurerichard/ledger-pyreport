...
def trial_balance_raw(ledger, date, pstart, label=None):
    ...
    for transaction in ledger.transactions:
        ...
        for posting in transaction.postings:
            if (
                posting.account.is_income or posting.account.is_expense
            ) and transaction.date < pstart:
                tb.balances[config["retained_earnings"]] = (
                    tb.get_balance(ledger.get_account(config["retained_earnings"]))
                    + posting.amount
                )
            elif posting.account.is_oci and transaction.date < pstart:
                tb.balances[config["accumulated_oci"]] = (
                    tb.get_balance(ledger.get_account(config["retained_earnings"]))
                    + posting.amount
                )
            else:
                tb.balances[posting.account.name] = (
                    tb.get_balance(posting.account) + posting.amount
                )
...
def trial_balance(ledger, date, pstart, commodity, label=None):
    tb_date, r_date = _add_unrealized_gains(
        trial_balance_raw(ledger, date, pstart, label=label), commodity
    )
    tb_pstart, r_pstart = _add_unrealized_gains(
        trial_balance_raw(ledger, pstart - timedelta(days=1), pstart), commodity
    )
    ...
    for account in set(list(r_date.keys()) + list(r_pstart.keys())):
        if account in r_pstart:
            for trn in r_pstart[account]:
                ...
                tb_date.balances[account.name] = (
                    tb_date.get_balance(account) + trn.postings[0].amount
                )

                if (
                    trn.postings[1].account.is_income
                    or trn.postings[1].account.is_expense
                ):
                    tb_date.balances[config["retained_earnings"]] = (
                        tb_date.get_balance(
                            ledger.get_account(config["retained_earnings"])
                        )
                        - trn.postings[0].amount
                    )
                elif trn.postings[1].account.is_oci:
                    tb_date.balances[config["accumulated_oci"]] = (
                        tb_date.get_balance(
                            ledger.get_account(config["accumulated_oci"])
                        )
                        - trn.postings[0].amount
                    )
                else:
                    tb_date.balances[trn.postings[1].account.name] = (
                        tb_date.get_balance(trn.postings[1].account)
                        - trn.postings[0].amount
                    )
                ...
                tb_date.balances[account.name] = (
                    tb_date.get_balance(account) + trn_reversal.postings[0].amount
                )
                tb_date.balances[trn_reversal.postings[1].account.name] = (
                    tb_date.get_balance(trn_reversal.postings[1].account)
                    - trn_reversal.postings[0].amount
                )
...
def _add_unrealized_gains(tb, commodity):
    ...
    for account in list(tb.ledger.accounts.values()):
        ...
        for amount in tb.get_balance(account).amounts:
            ...
            amt_market = amount.exchange(
                commodity, False, date=tb.date, ledger=tb.ledger
            )
        ...
        if unrealized_gain != 0:
            ...
            transaction.postings.append(
                Posting(transaction, unrealized_gain_account, -unrealized_gain)
            )
        ...
        if unrealized_loss != 0:
            ...
            transaction.postings.append(
                Posting(transaction, unrealized_loss_account, -unrealized_loss)
            )
            tb.ledger.transactions.append(transaction)
...
def balance_sheet(tb):
    ...
    total_pandl = tb.get_total(
        tb.ledger.get_account(config["income_account"])
    ) + tb.get_total(tb.ledger.get_account(config["expenses_account"]))

    # Add Current Year Earnings account
    tb.balances[config["current_year_earnings"]] = (
    ...
    tb.balances[config["current_year_oci"]] = (
        tb.get_balance(tb.ledger.get_account(config["current_year_oci"])) + total_oci
    )
...
def ledger_to_cash(ledger, commodity):
    for account in list(ledger.accounts.values()):
        if not (
            account.is_cash
            or account.is_income
            or account.is_expense
            or account.is_equity
        ):
...
def account_flows(ledger, date, pstart, accounts, related, label=None):
    transactions = [
        ...
        if any(p.account in accounts for p in t.postings)
        and t.date <= date
        and t.date >= pstart
    ]
