...
class Ledger:
    ...
    def get_price(self, commodity_from, commodity_to, date):
        prices = [
            ...
            if p[1] == commodity_from.name
            and p[2].commodity == commodity_to
            and p[0].date() <= date.date()
        ]
...
class Transaction:
    def __init__(
        self, ledger, id, date, description, code=None, uuid=None, metadata=None
    ):
        self.ledger = ledger
        self.id = id
        self.date = date
    ...
    def reverse(self, id, date, description, code=None, uuid=None):
        result = Transaction(
            self.ledger, id, date, description, code, uuid, self.metadata
        )
        result.postings = [
            Posting(p.transaction, p.account, -p.amount, p.comment, p.state)
    ...
    def exchange(self, commodity):
        ...
        for k, g in itertools.groupby(
            self.postings, key=lambda p: (p.account, p.comment, p.state)
        ):
            account, comment, state = k
            posting = Posting(
                result,
                account,
                sum(p.exchange(commodity).amount for p in g),
                comment,
                state,
            )
    ...
    def split(self, report_commodity):
        ...
        for posting in self.postings:
            base_amount = posting.amount.exchange(
                report_commodity, True
            ).amount  # Used to apportion other commodities
            ...
            for unbalanced_posting in unbalanced_postings[:]:
                if math.copysign(1, amount_to_balance) != math.copysign(
                    1, unbalanced_posting[2]
                ):
                    if abs(unbalanced_posting[2]) == abs(amount_to_balance):
                        ...
                        result.append(
                            (
                                ...
                                Posting(
                                    self,
                                    unbalanced_posting[0].account,
                                    Amount(
                                        -amount_to_balance
                                        / unbalanced_posting[1]
                                        * unbalanced_posting[0].amount.amount,
                                        unbalanced_posting[0].amount.commodity,
                                    ),
                                    unbalanced_posting[0].comment,
                                    unbalanced_posting[0].state,
                                ),
                            )
                        )
                        amount_to_balance = 0
    ...
    def perspective_of(self, account):
        ...
        if not all(
            p.amount.sign == acc_postings[0].amount.sign for p in acc_postings[1:]
        ):
        ...
        opp_postings = [
            p for p in self.postings if p.amount.sign == -acc_postings[0].amount.sign
        ]
        ...
        new_opp_postings = [
            Posting(
                self,
                opp_postings[0].account,
                -p.amount,
                opp_postings[0].comment,
                opp_postings[0].state,
            )
            for p in acc_postings
        ]
...
class Posting:
    ...
    def __init__(
        self, transaction, account, amount, comment=None, state=State.UNCLEARED
    ):
    ...
    def exchange(self, commodity):
        ...
        return Posting(
            self.transaction,
            self.account,
            self.amount.exchange(commodity, True),
            self.comment,
            self.state,
        )  # Cost basis
...
class Amount:
    ...
    def tostr(self, round=True):
        if self.commodity.is_prefix:
            amount_str = ("{}{:.2f}" if round else "{}{}").format(
                self.commodity.name, self.amount
            )
        else:
            amount_str = ("{:.2f} {}" if round else "{} {}").format(
                self.amount, self.commodity.name
            )
    ...
    def exchange(self, commodity, is_cost=True, price=None, date=None, ledger=None):
        ...
        if (
            is_cost
            and self.commodity.price
            and self.commodity.price.commodity.name == commodity.name
        ):
        ...
        if date and ledger:
            if any(p[1] == self.commodity.name for p in ledger.prices):
                ...
                return self.exchange(
                    commodity,
                    is_cost,
                    ledger.get_price(self.commodity, commodity, date),
                )
            else:
...
class Balance:
    ...
    def strip_prices(self):
        ...
        for amount in self.amounts:
            new_amount = next(
                (a for a in new_amounts if a.commodity.name == amount.commodity.name),
                None,
            )
            if new_amount is None:
                new_amounts.append(
                    Amount(amount.amount, amount.commodity.strip_price())
                )
            else:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Balance):
            for amount in other.amounts:
                new_amount = next(
                    (a for a in new_amounts if a.commodity == amount.commodity), None
                )
                if new_amount is None:
            ...
            new_amount = next(
                (a for a in new_amounts if a.commodity == other.commodity), None
            )
            if new_amount is None:
...
class Commodity:
    ...
    def __repr__(self):
        return "<Commodity {} ({})>".format(
            self.name, "prefix" if self.is_prefix else "suffix"
        )
...
class TrialBalance:
    ...
    def get_total(self, account):
        return self.get_balance(account) + sum(
            (self.get_total(a) for a in account.children), Balance()
        )
